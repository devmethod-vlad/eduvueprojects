  ...
  <div class="vue-survey-container">
    ...
    <div v-else class="vue-survey-workspace">
      ...
      <template v-else>
        ...
        <template v-if="attemptsText!=='' && !surveyEndTime">
...
import {
  CheckAccessError, CheckDataError, CheckUserAccesError, FormSurveyError,
  GetIdError,
  GetSurveyInfoError, PrepareItemsError, ProcessLogicError,
  SetParamsError
...
const checkAccess = async()=>{
  ...
  console.log("checkAccess accResp: ", accResp);
...
const processSurvey = async()=>{
  ...
  survey.value = await new Model(surveyContent.value);

  survey.value.locale = "ru";

  await console.log("processSurvey survey: ", survey.value);

  survey.value.onComplete.add(onCompleteSurvey);
...
const saveAndUpdateState = async(action)=>{
  // console.log("saveAndUpdateState");
  try {
    let updateResp = await fetch(app_config.apiUrl+app_config.surveyApiUrl, {
      ...
      body: JSON.stringify({
        ...
        "starttime": workspaceParams.value.startime
      })
    });

    let updateAnswer = await updateResp.json();

    // await console.log("saveAndUpdateState updateAnswer: ", updateAnswer);
...
const showAnswer = async(resultObject, endtime=false)=>{
  ...
  if(Number(workspaceParams.value.showCustomresultsPanel)===1){
    ...
    if(Number(workspaceParams.value.checkResult) === 1 && Number(workspaceParams.value.showResult) === 1){
      ...
      if (endtime){
        resultText.value = 'Отведенное время истекло. Cпасибо за участие! Ваши ответы учтены!';
      }
      else {
        resultText.value = 'Cпасибо за участие! Ваши ответы учтены!';
      }
    }
    if(resultText.value !== ''){
...
onMounted(async ()=>{
  try {
    ...
    await console.log("dbSurveyInfo: ", dbSurveyInfo.value);
    ...
    if(!userAccessInfo.showSurvey){
      ...
      console.log("surveyContent.value: ", surveyContent.value);

      workspaceParams.value = await workspaceData['workspaceParams'];

      console.log("workspaceParams.value: ", workspaceParams.value);
      ...
      if (Object.keys(workspaceParams.value).length !== 0){
        ...
        await console.log("questionsInfo: ", questionsInfo.value);
        ...
        await console.log("formSurvey surveyData: ", surveyData.value);
        ...
        if(Number(workspaceParams.value.use_raw_content)===0){
        ...
        await console.log("Логика исполнения");
        //Логика исполнения
        if (Number(workspaceParams.value.surveyHasState) === 0 && Number(workspaceParams.value.saveProgress) === 0){
          ...
          // console.log("surveyLogic surveyHasState=1 & saveProgress=1");
          ...
          if(answer['status']==='ok'){
            ...
            if (answer['info'].surveyinfo !==''){
              surveyContent.value = await JSON.parse(answer['info'].surveyinfo);
            }
            ...
            if (Number(answer['info'].timexecute) === 0 && workspaceParams.value.surveyTime !== 0){
              ...
              surveyContent.value.maxTimeToFinish = await Number(answer['info'].timexecute);
              ...
              updateVar.value = setInterval(()=>{updateTimer(true)}, workspaceParams.value.updateInterval);
