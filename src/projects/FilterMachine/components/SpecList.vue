<template>
  <ModalWindow
      modal-size="large"
      @closemodal="resetState();emits('closespeclist')"
  >
    ...
    <template v-else>
      ...
      <div class="fm-list-wr">
        <div class="fm-list-title">
          Выбор Должности (отметьте те должности, которые будут участвовать в формировании списка)
        </div>
        ...
        <div class="em-search-wr" :class="{'em-disabled-style': isLoading}">
          ...
          <div class="em-search-input-wr">
            ...
            <input @keyup.enter="searchProcess()" placeholder="Поиск по названию должности"
        ...
        <div class="em-table-pagecount-container" :class="{'em-disabled-style': isLoading}">
          ...
          <div class="fm-list-table-download-wr">
            <button @click="downloadReport('specall')" class="vue-btn vue-btn-normal"
                    :class="{'em-disabled-style': exportActive}"
            >
              Скачать все
            </button>
            <button @click="downloadReport('specactive')" v-if="activeList.length !==0 || inactiveList.length !==0"
          ...
          <div class="fm-list-table">
            ...
            <div class="fm-list-table-row">
              ...
              <div class="fm-list-item fm-list-item-header name-item">
                ...
                <div v-if="Number(totalItems) >1" @click="processSort()"
            ...
            >
              ...
              <div class="fm-list-table-row">
                <div class="fm-list-item checkbox-item">
                  <input :id="'fm-list-speccheckbox-'+item.id.toString()" :value="Number(item.id)"
                         name="fm-list-speccheckbox" @change="processCheckbox($event, item.id)"
...
import {getSpecList} from "@/projects/FilterMachine/services/getspeclist";
...
// EMITS
const emits = defineEmits(['closespeclist']);
...
const downloadReport = async(expType)=>{
  ...
  try{
    ...
    if(expResp.status.toString() !== '200'){
      ...
      if(expType === "specall"){
        link.download =  "Все_должности_"+dayjs().format('DD_MM_YYYY')+".xlsx";
      }
      else if(expType === "specactive"){
        link.download =  "Отмеченные_должности_"+dayjs().format('DD_MM_YYYY')+".xlsx";
      }
...
const syncStorage = ()=>{
  ...
  if (fmStoreCopy.specState.activeAll !== activeAll.value){
    needResetTempData = true;
  }
  if (!compareArrays(fmStoreCopy.specState.activeList.sort(), activeList.value.sort())){
    needResetTempData = true;
  }
  if (!compareArrays(fmStoreCopy.specState.inactiveList.sort(), inactiveList.value.sort())){
    needResetTempData = true;
  }


  fmStoreCopy.specState.activeAll = activeAll.value;
  fmStoreCopy.specState.activeList = activeList.value;
  fmStoreCopy.specState.inactiveList = inactiveList.value;
  ...
  if(fmStore.$state.saveState) {
    localStorage.setItem('em-fm-speсs', JSON.stringify(localStore));
  }
...
const processCheckbox = (event, specid)=>{

  if(event.target.checked){
    if(!activeAll.value){
      if (!activeList.value.includes(Number(specid))){
        activeList.value.push(Number(specid))
      }
    }
    else {
      if (inactiveList.value.includes(Number(specid))){
        let findIndex = inactiveList.value.indexOf(Number(specid));
        if (findIndex !== -1){
    ...
    if(activeAll.value){
      if (!inactiveList.value.includes(Number(specid))){
        inactiveList.value.push(Number(specid))
      }
    }
    else{
      if (activeList.value.includes(Number(specid))){
        let findIndex = activeList.value.indexOf(Number(specid));
        if (findIndex !== -1){
...
const initLocalList = ()=>{
  ...
  if(fmStore.$state.saveState){
    ...
    if (localStorage.getItem('em-fm-specs')){
      ...
      let localList = JSON.parse(localStorage.getItem('em-fm-specs'));

      let specList = localList.list;
...
const getList = async()=>{
  ...
  let answer = await getSpecList(requestParams.value);
...
const requestParams = computed(()=>{
  ...
  return {
    ...
    'spec_activeall': activeAll.value ? 1 : 0,
    'spec_activelist': activeList.value,
    'spec_inactivelist': inactiveList.value,
    'spec_searchinput': searchInput.value,
    'spec_activepage': activePage.value,
    'spec_sorting': sorting.value,
    'spec_filters': filters.value,
    'spec_pagecount': pageCount.value
  }
